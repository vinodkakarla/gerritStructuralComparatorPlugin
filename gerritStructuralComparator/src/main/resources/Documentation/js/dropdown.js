...
(function($) {
	...
	"dropdown": function() {
		...
		select.after(div).hide().data("dropdown.initialized", true);
	    });
		...
		dropdown.
		    find("div[role='selected'] a").click(function() {
			$(".dropdown").not(dropdown).removeClass("dropdownActive");
			dropdown.toggleClass("dropdownActive");
			return false;
		    }).end().
		    find("div[role='options'] ul li a").click(function() {
			dropdown.removeClass("dropdownDefaultValue").
			    find("div[role='selected'] a span.label").html($(this).html()).end().
		...
		if (select && selected.length > 0) {
		    $(dropdown.find("div[role='options'] ul li a")[select.children().index(selected)]).click();
		}
